...
for path in [
    './test/', './main/d96/parser/', './main/d96/utils/', './main/d96/astgen/',
...
def main(argv):
    if len(argv) < 1:
        ...
        subprocess.run(
            [
                "java", "-jar", ANTLR_JAR, "-o", "../target", "-no-listener",
                "-visitor", "main/d96/parser/D96.g4"
            ]
        )
    elif argv[0] == 'clean':
        ...
        if not os.path.isdir(TARGET_DIR + "/" + GENERATE_DIR):
            subprocess.run(
                [
                    "java", "-jar", ANTLR_JAR, "-o", GENERATE_DIR,
                    "-no-listener", "-visitor", "main/d96/parser/D96.g4"
                ]
            )
