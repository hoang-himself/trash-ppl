...
if os.path.isdir(
    '../target/main/d96/parser'
) and not '../target/main/d96/parser/' in sys.path:
...
class TestCodeGen():
    ...
    def check(soldir, asttree, num):
        ...
        try:
            ...
            subprocess.call(
                "java  -jar " + JASMIN_JAR + " " + path + "/D96Class.j",
                shell=True,
                stderr=subprocess.STDOUT
            )

            subprocess.run(
                "java -cp ./lib:. D96Class", shell=True, stdout=f, timeout=10
            )
        except StaticError as e:
            f.write(str(e))
